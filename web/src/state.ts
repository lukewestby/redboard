...
export const username$ = atom<string | null>({
  ...
  default: null,
  effects: [
    syncEffect({ refine: nullable(string()), storeKey: 'LocalStorage' })
  ]
})
...
export const sessionId$ = atom<string | null>({
  ...
  default: null,
  effects: [
    syncEffect({ refine: nullable(string()), storeKey: 'SessionStorage' }),
  ]
})
...
export const objectIds$ = atomFamily<Set<string>, string>({
  key: 'ObjectIds',
  default: new Set(),
  effects: (boardId) => [
    syncEffect({
...
export const sessionIds$ = atom<Set<string>>({
  key: 'SessionIds',
  default: new Set(),
  effects: [
    syncEffect({
...
export const cursors$ = atomFamily<Point | null, string>({
  ...
  effects: (sessionId) => [
    syncEffect({
      refine: nullable(object({ x: number(), y: number() })),
